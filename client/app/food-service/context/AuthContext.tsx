      ...
      if (storedToken && storedUserData) {
        try {
          ...
          const normalizedUserData = {
            ...userData,
            // API /auth/profile hiện tại chưa trả về role, nên giữ role customer để tránh clear session sai.
            role: userData?.role || 'customer',
          };

          if (normalizedUserData?.role !== 'customer') {
    ...
    const loadUser = async () => {
      if (token && !currentUser) {
        try {
          ...
          if (storedUserData) {
            try {
              storedRole = JSON.parse(storedUserData)?.role;
            } catch (parseError) {
          ...
          const normalizedUser = {
            ...user,
            role: user?.role || storedRole || 'customer',
          };
