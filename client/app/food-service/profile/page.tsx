...
const Profile = () => {
  ...
  const normalizeAndSyncUserData = (incomingUser: UserData): UserData => {
    let storedUserData: Partial<UserData> = {};
    const rawStoredUserData = localStorage.getItem('userData');

    if (rawStoredUserData) {
      try {
        storedUserData = JSON.parse(rawStoredUserData);
      } catch (parseError) {
        console.error('Error parsing local user data before sync:', parseError);
      }
    }
    ...
    const mergedUserData: UserData = {
      ...storedUserData,
      ...incomingUser,
      role: incomingUser.role || storedUserData.role || currentUser?.role || 'customer',
    };

    const authUserPayload: Record<string, any> = {
      ...currentUser,
      ...mergedUserData,
      role: mergedUserData.role || 'customer',
    };

    const normalizedId = mergedUserData.id || mergedUserData.user_id || currentUser?.id;
    if (normalizedId !== undefined && normalizedId !== null) {
      authUserPayload.id = String(normalizedId);
    }

    localStorage.setItem('userData', JSON.stringify(authUserPayload));
    setCurrentUser(authUserPayload as any);
    ...
    try {
      ...
      if (storedUserData) {
        try {
          ...
          const normalizedStoredData = normalizeAndSyncUserData(processedData);
          setUserData(normalizedStoredData);
          setLoading(false);
        } catch (parseError) {
      ...
      const normalizedApiData = normalizeAndSyncUserData(apiUserData);
      setUserData(normalizedApiData);
      setError(null);
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const updatedUserData = normalizeAndSyncUserData({
        ...(userData || {}),
        avatar_url: imageUrl,
      });
